  ...
  return (
    <footer className="w-full bg-[#040919] text-white pt-12 pb-6 font-roboto">
      <div className="container mx-auto px-4 md:px-0 max-w-[1140px]">
        {/* Dynamic Grid based on the columns passed in */}
        <div
          className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-${columns.length} gap-8 mb-12`}
        >
              ...
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link, i) => (
                  <li key={i} className="flex items-center text-left">
                    <ChevronRight className="w-3.5 h-3.5 mr-2 text-gray-400 shrink-0 mt-0.5" />
                    <Link
                      href={link.url}
                      className="text-[14px] text-gray-300 hover:text-[#e83e8c] transition-colors leading-snug"
                    >
                      {link.text}
                    </Link>
                  </li>
        ...
        <div className="border-t border-gray-800/80 pt-6 flex justify-center text-center">
          <p className="text-[14px] text-gray-400 font-medium">
            All rights reserved &copy; {new Date().getFullYear()}, {facultyName}
