...
const stats = [
  ...
  },
  {
    ...
    title: "Facebook",
    value: null,
    icon: GraduationCap, // I updated this to use the actual Facebook icon if you have it imported
...
export default function StatsBar() {
  return (
    <section className="w-full bg-gray-50 flex items-center py-10 lg:py-0 lg:h-[150px] animate-in fade-in slide-in-from-bottom-8 duration-1000 transition-colors duration-300 dark:bg-slate-950 mx-auto">
      <div className="container px-6 mx-auto md:px-0 max-w-[1140px]">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-6 w-full">
          {stats.map((stat) => {
            ...
            return stat.href ? (
              <a
                ...
                className="block h-full focus:outline-none focus:ring-2 focus:ring-ru-blue rounded-3xl w-full"
              >
                {CardContent}
              </a>
            ) : (
              <div key={stat.id} className="w-full h-full">
                {CardContent}
              </div>
            );
          })}
